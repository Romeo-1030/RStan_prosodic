...
sbc_word_perdoc_freq = sbc %>%
  ...
  filter(!text_lower %in% c("b","c","d","e","f","g","h","j","k","l","m","n","o","p","q","qu","r","s","t","u","v","w","x","y","z","ph","th","ch","sh","bl","br","cl","cr","dr","fl","fr","gl","gr","kn","pl","pr","sc","scr","sk","sl","sm","sn","sp","spr","st","str","sw","tr","wh","wr")) %>% #skip
  count(.drop = F) %>%
  ungroup %>%
...
write_csv(
  sbc_word_perdoc_freq %>% arrange(Urank),
  here("data", "sbc_word_perdoc_freq.csv")
)
...
sbc = sbc %>%
  ...
  ungroup
sbc = sbc %>%
  group_by(docId, pSentSeq) %>%
  mutate(pSentWordLen = sum(kind == "word")) %>%
  mutate(
    pSentPlace = put_values_in_true(
      pythonlike_range(pSentWordLen[1]),
      kind == "word"
    )
  ) %>%
  ungroup
...
sbc = sbc %>%
  group_by(docId, pSentSeq) %>%
  mutate(pSentUnitLen = length(unique(unitId))) %>%
  ungroup
sbc = sbc %>%
  group_by(docId, pSentSeq) %>%
  mutate(pSentUnitPlace = rank_unique(unitId)) %>%
  ungroup
...
get_staircase("oh", pSentUnitPlace, pSentUnitLen, max_len = 25)
```
