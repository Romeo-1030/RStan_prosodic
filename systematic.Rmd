...
unit_length <- 5
max_length <- 8
hurdle <- 0  # Test with hurdle = 0

# Call the function and check the normalization
result <- trun_hurdle(theta, lambda, psi_intercept, psi_slope, alpha = NULL, unit_length, max_length, hurdle)
...
```{r}
hurdle_neg2_with = getModel("with", "Stan/hurdle_soft_test.stan", back = T)
```

```{r}
with_hurdle = Hurdle_Pois_Quantities_rev(hurdle_neg2_with, with, back = T, hurdle = -2)
```
...
```{r}
p3_6 <- plotting(all_df, result, 'with')
...
```{r}
hurdle_1_should = getModel("should", "Stan/hurdle_soft_test.stan", back = F)
```
...
```{r}
hurdle_1_should = getModel("should", "Stan/gen_pois_hurdle2_rev.stan", back = F)
```
...
```{r}
her_final = Gen_Pois_Quantities(gen_pois_her, her, back = T)
```
...
```{r}
hur_pois_if = getModel("if", "Stan/gen_pois_hurdle.stan", back = F)
```

```{r}
if_hurdle = Hurdle_Pois_Quantities(hur_pois_if, if_, back = F, hurdle = 0)
```

```{r}
output <- df_visual(if_hurdle, if_)
all_df <- output$all_df
result <- output$result
```
...
```{r}
p18_5 <- plotting(all_df, result, 'if')
print(p18_5)
```
...
```{r}
very <- sbc %>%
  filter(tolower(text) == 'very') %>%
  filter(!is.na(place))
```

```{r}
hurdle_neg2_very = getModel("very", "Stan/hurdle_soft_test.stan", back = F)
```

```{r}
very_hurdle = Hurdle_Pois_Quantities_rev(hurdle_neg2_very, very, back = F, hurdle = -2)
```

```{r}
output <- df_visual(very_hurdle, very)
all_df <- output$all_df
result <- output$result
```
...
```{r}
p3_6 <- plotting(all_df, result, 'very')
...
```{r}
hurdle_1_do = getModel("do", "Stan/gen_pois_hurdle2.stan", back = F)
```

```{r}
do_hurdle = Hurdle_Pois_Quantities(hurdle_1_do, do, back = F, hurdle = 1)
```

```{r}
output <- df_visual(do_hurdle, do)
all_df <- output$all_df
result <- output$result
```
...
```{r}
d <- sbc %>%
  filter(tolower(text) == "'d") %>%
  filter(!is.na(place))
```


```{r}
hurdle_1_d = getModel("'d", "Stan/gen_pois_hurdle2.stan", back = F)
```

```{r}
d_hurdle = Hurdle_Pois_Quantities(hurdle_1_d, d, back = F, hurdle = 1)
```

```{r}
output <- df_visual(d_hurdle, d)
all_df <- output$all_df
result <- output$result
```
...
```{r}
samples <- extract(hurdle_1_d)

if (is.vector(d_hurdle$nb_place)) {
  d_hurdle$nb_place <- matrix(d_hurdle$nb_place, ncol = 1)
}

observed_variance <- var(d$place_minus_1)
predicted_variances <- apply(d_hurdle$nb_place, 2, var)
mean_predicted_variance <- mean(predicted_variances)

# Overdispersion ratio
overdispersion_ratio <- observed_variance / mean_predicted_variance
...
```{r}
hurdle_1_ll = getModel("'ll", "Stan/gen_pois_hurdle2.stan", back = F)
```

```{r}
ll_hurdle = Hurdle_Pois_Quantities(hurdle_1_ll, ll, back = F, hurdle = 1)
```

```{r}
output <- df_visual(ll_hurdle, ll)
all_df <- output$all_df
result <- output$result
```
...
```{r}
this <- sbc %>%
  filter(tolower(text) == "this") %>%
  filter(!is.na(place))
```


```{r}
mix_this = getModel("this", "Stan/mixture_model.stan", back = F)
```

```{r}
this_final_mix = Mix_Pois_Quantities(mix_this, this)
```

```{r}
output <- df_visual(this_final_mix, this)
all_df <- output$all_df
result <- output$result
```
...
```{r}
p31 <- plotting(all_df, result, "this")
print(p31)
```
...
```{r}
that <- sbc %>%
  filter(tolower(text) == "that") %>%
  filter(!is.na(place))
```


```{r}
mix_that = getModel("that", "Stan/mixture_model.stan", back = F)
```

```{r}
that_final_mix = Mix_Pois_Quantities(mix_that, that)
```

```{r}
output <- df_visual(that_final_mix, that)
all_df <- output$all_df
result <- output$result
```
...
```{r}
p32 <- plotting(all_df, result, "that")
print(p32)
```
...
```{r}
rev_it = getModel("it", "Stan/test_rev.stan", back = F)
```

```{r}
it_final = Hurdle_Pois_Quantities_rev(rev_it, it, back = F, hurdle = -1)
```

```{r}
output <- df_visual(it_final, it)
all_df <- output$all_df
result <- output$result
```
...
```{r}
p33 <- plotting(all_df, result, "it")
print(p33)
```
## every
```{r}
every <- sbc %>%
  filter(tolower(text) == "every") %>%
  filter(!is.na(place))
```


```{r}
rev_every = getModel("every", "Stan/gen_pois_hurdle2_rev.stan", back = F)
```

```{r}
every_final = Hurdle_Pois_Quantities_rev(rev_every, every, back = F, hurdle = -2)
```

```{r}
output <- df_visual(every_final, every)
all_df <- output$all_df
result <- output$result
```
...
```{r}
p34 <- plotting(all_df, result, "every")
print(p34)
```
...
```{r}
there <- sbc %>%
  filter(tolower(text) == "there") %>%
  filter(!is.na(place))
```


```{r}
mix_there = getModel("there", "Stan/mixture_model.stan", back = F)
```

```{r}
there_final_mix = Mix_Pois_Quantities(mix_there, there)
```

```{r}
output <- df_visual(there_final_mix, there)
all_df <- output$all_df
result <- output$result
```
...
```{r}
p35 <- plotting(all_df, result, "there")
print(p35)
```
