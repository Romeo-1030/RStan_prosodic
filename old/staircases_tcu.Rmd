...
sbc = sbc %>% group_by(docId, pSentSeq) %>% mutate(pSentUnitPlace = rank_unique(unitId)) %>% ungroup
...
```{r}
get_staircase("person", pSentUnitPlace, pSentUnitLen, y_text = "TCU length", x_text = "Place in TCU", max_len = 25)
get_staircase("people", pSentUnitPlace, pSentUnitLen, y_text = "TCU length", x_text = "Place in TCU", max_len = 25)
...
```{r}
for(word in sbc_top200){
    get_staircase(word, pSentUnitPlace, pSentUnitLen, y_text = "TCU length", x_text = "Place in TCU", max_len = 25)
